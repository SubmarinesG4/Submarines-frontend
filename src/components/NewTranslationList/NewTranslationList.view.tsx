		...
		<Box sx={{ width: "auto", padding: "1em 1em" }} role="presentation">
			...
			<FormGroup>
				<form onSubmit={handleSubmit(handleFormSubmit)}>
					...
					>
						<FormControlLabel
							control={<Checkbox defaultChecked={false} {...register("published")} />}
							label="Pubblicato"
						/>
					</Box>
					...
					<Box sx={{ borderBottom: 1, borderColor: "divider", paddingTop: "1em" }}>
						<Tabs value={value} onChange={handleTabChange} aria-label="basic tabs example">
							<Tab label={logic.defaultTranslationLanguage + " (default)"} key={0} {...a11yProps(0)} />
							{logic.languages
								.filter((language) => language !== logic.defaultTranslationLanguage)
								.map((language, index) => {
									return <Tab label={language} key={index + 1} {...a11yProps(index + 1)} />;
								})}
						</Tabs>
					</Box>
						...
						.map((language, index) => {
							return (
								<TabPanel value={value} index={index + 1} key={index + 1}>
									<TextField id={"textarea-language-" + language} label="" multiline rows={4} {...register(language)} />
								</TabPanel>
							);
						})}
