...
export default function View(props: TranslationTableProps) {
	...
	const { data, isLoading, error } = useTranslationTable({ filter: queryFilter, tenantName: props.tenantName });
	...
	const handleTableRendering = () => {
		if (isLoading) {
			...
			return <Box sx={{ margin: "1em" }}>Errore nel fetch delle traduzioni</Box>;
		} else {
			return (
											...
											return (
												<TableRow hover role="checkbox" tabIndex={-1} key={row.translationKey}>
													...
													<TableCell key={"actions"} align={columns[2].align}>
														...
														>
															<IconButton sx={{ marginRight: "0.2em" }} onClick={props.showEdit(row.translationKey)}>
																<EditIcon />
															</IconButton>
															<IconButton onClick={props.showHistory(row.translationKey)}>
																<HistoryIcon />
															</IconButton>
														</Box>
													</TableCell>
