...
import { useGetTenantQuery, useUpdateTenantMutation } from "@/app/services/tenantsApiSlice";
import CancelIcon from "@mui/icons-material/Cancel";

const languages = ["en", "de", "it", "es", "fr"];

export default function View(props: EditTenantFormProps) {
	const { data: tenant, isLoading } = useGetTenantQuery({ id: props.tenantName });
	const { register, handleSubmit, control } = useForm({
		defaultValues: { languages: [], numberOfTranslations: 200, defaultLanguage: "" },
	});
	...
	const handleFormSubmit = (data: any) => {
		updateTenant({
			...
			defaultTranslationLanguage: tenant.defaultTranslationLanguage,
		})
			.unwrap()
			.then((payload) => {
				console.log(payload);
	...
	if (isLoading) return <Box sx={{ width: "auto", padding: "1em 1em" }}>Loading...</Box>;
	return (
		<Box sx={{ width: "auto", padding: "1em 1em" }}>
			<form onSubmit={handleSubmit(handleFormSubmit)}>
				...
				<Box display="flex" justifyContent="flex-end" alignItems="flex-end">
					<Button variant="outlined" type="button" sx={{ marginX: "0.5em" }} onClick={props.closeDrawer}>
						Chiudi
					</Button>
